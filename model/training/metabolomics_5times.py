...
def run_experiment(config, run_seed):
    ...
    # 2f. Generate Scores for All Data Splits and SHAP values for EXTERNAL TEST SET
    ...
    scores_path = os.path.join(results_dir, "Scores", config['name'], str(SEED))
    os.makedirs(scores_path, exist_ok=True)
    
    dataloaders_to_process = {
        'train': train_loader, 'val': val_loader,
        'internal_test': internal_test_loader, 'external_test': external_test_loader
    }

    for data_split, dataloader in dataloaders_to_process.items():
        logger.info(f"Calculating scores for '{data_split}' set...")
        _, _, final_outputs, _, _ = trainer._run_epoch(dataloader, is_training=False)
        eids_for_scores = dataloader.dataset.master_eids.values
        scores_df = pd.DataFrame(eids_for_scores, columns=['eid'])
        for outcome in config['outcomes_list']:
            scores_df[outcome] = final_outputs[outcome].numpy().flatten()
        scores_filename = os.path.join(scores_path, f"{data_split}_scores_{config['predictor_set']}.csv")
        scores_df.to_csv(scores_filename, index=False)
        logger.info(f"Saved scores for {data_split} set to {scores_filename}")

    logger.info("\n--- Preparing for SHAP analysis on External Test Set ---")
    background_data = generate_background_samples(train_loader, num_samples=500)
    all_test_samples = [x_data for _, x_data, _, _ in external_test_loader]
    test_data_tensor = torch.cat(all_test_samples, dim=0).to('cpu')
    external_test_eids = torch.cat([eid for eid, _, _, _ in external_test_loader]).numpy()
    ...
    for outcome in config['outcomes_list']:
        ...
        if len(external_test_eids) == len(shap_df):
            shap_df.insert(0, 'eid', external_test_eids)
            logger.info(f"Successfully added 'eid' column to SHAP DataFrame for {outcome}.")
        else:
            logger.warning(f"Could not add 'eid' column for {outcome}: Mismatch in length between eids ({len(external_test_eids)}) and SHAP rows ({len(shap_df)}).")
...
if __name__ == '__main__':
    ...
    config_path = '/your path/cardiomicscore/config/Metabolomics/final_param.json'
    try:
