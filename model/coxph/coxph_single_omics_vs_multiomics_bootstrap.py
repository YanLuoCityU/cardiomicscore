...
class MultiOmicsBootstrapEvaluator:
    """
    Compares single-omics models against multi-omics models by calculating
    bootstrapped confidence intervals for the incremental improvement in C-index.
    """
    ...
    def _load_model_data(self, predictor_list: list, outcome: str, data_split: str) -> pd.DataFrame:
        ...
        for predictor in predictor_list:
            ...
            try:
                if predictor in ['AgeSex', 'Clinical', 'PANEL', 'Genomics']:
    ...
    def run_analysis(self):
        """Orchestrates the bootstrap evaluation for all single- vs multi-omics comparisons."""
            ...
            for base_combo in baseline_combinations:
                ...
                for i in range(1, len(remaining_omics) + 1):
                    for subset in itertools.combinations(remaining_omics, i):
                        ...
                        self.logger.info(f"\n----- Bootstrapping: {comparison_combo} vs. {base_combo} -----")
                        ...
                        metrics_ci = self._run_bootstrap_on_test_set(df_base_common, df_comparison_common)
                
                        if metrics_ci:
                            metrics_ci['outcome'] = outcome
                            metrics_ci['baseline_model'] = '_'.join(sorted(base_combo, key=lambda p: custom_sort_order.index(p)))
                            metrics_ci['comparison_model'] = '_'.join(comparison_combo)
        ...
        if all_results:
            ...
            results_filename = os.path.join(self.save_dir, "cindex_single_vs_multi_omics_bootstrap_ci_summary.csv")
...
if __name__ == '__main__':
    ...
    log_filename = os.path.join(LOG_DIR, "CoxPH/Bootstrap_CI_Single_vs_Multi_Omics.log")
    os.makedirs(os.path.dirname(log_filename), exist_ok=True)
    logging.basicConfig(level=logging.INFO, 
                        handlers=[logging.FileHandler(log_filename, mode='w'), logging.StreamHandler(sys.stdout)],
                        format='%(asctime)s - %(levelname)s - %(message)s')
    logger_main = logging.getLogger("MultiOmicsBootstrapScript")
    
    evaluator = MultiOmicsBootstrapEvaluator(
        data_dir=DATA_DIR, 
