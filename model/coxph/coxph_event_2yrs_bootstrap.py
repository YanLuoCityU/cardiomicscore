...
class EventExclusionBootstrapEvaluator:
    """
    Performs a bootstrapped evaluation of CoxPH models after excluding subjects
    ...
    def _load_model_data(self, predictor_list: list, outcome: str, data_split: str) -> pd.DataFrame:
        ...
        try:
            ...
            if num_excluded > 0:
                self.logger.info(f"Excluding {num_excluded} of {initial_count} samples with events occurring within 2 years for outcome '{outcome}'.")
        ...
        merged_df = outcome_df # Start merging with the filtered outcome data

        for predictor in predictor_list:
            x_df = pd.DataFrame()
            path = "N/A"
            try:
                if predictor in ['PANEL', 'Genomics', 'Metabolomics', 'Proteomics']:
                    ...
                    else: # Metabolomics or Proteomics
    ...
    def run_analysis(self):
        """Orchestrates the bootstrap evaluation for the specified PANEL-based combinations."""
        ...
        for outcome in outcomes_to_run:
            ...
            for combo in predictor_sets:
                self.logger.info(f"\n----- Bootstrapping Combo: {combo} vs. Baseline: {baseline_combo} -----")
                ...
                # Run the bootstrap process for this comparison on the event-excluded data
        ...
        if all_results:
            ...
            results_filename = os.path.join(self.save_dir, "cindex_event_2yrs_bootstrap_ci_summary.csv")
...
if __name__ == '__main__':
    ...
    log_filename = os.path.join(LOG_DIR, "CoxPH/Bootstrap_CI_Event_2yrs.log")
    os.makedirs(os.path.dirname(log_filename), exist_ok=True)
    logging.basicConfig(level=logging.INFO, 
                        handlers=[logging.FileHandler(log_filename, mode='w'), logging.StreamHandler(sys.stdout)],
                        format='%(asctime)s - %(levelname)s - %(message)s')
    logger_main = logging.getLogger("EventExclusionBootstrapScript")
    
    evaluator = EventExclusionBootstrapEvaluator(
        data_dir=DATA_DIR, 
