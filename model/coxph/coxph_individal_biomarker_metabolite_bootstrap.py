...
class BootstrapEvaluator:
    ...
    def _load_model_data(self, predictor_list: list, outcome: str, data_split: str) -> pd.DataFrame:
        ...
        protein_biomarkers = [
            "CDCP1", "HAVCR1", "LTBP2", "MMP12", "NTproBNP", "BCAN", "FASLG", "NEFL", 
            "PM20D1", "ART3", "NPPB", "ELN", "GALNT5", "KLK4", "PLB1", "CRISP3", "TIMD4"
        ]
        metabolite_biomarkers = [
            "MUFA", "Creatinine", "XXL_VLDL_CE", "IDL_FC", "XL_HDL_FC", "Albumin", "LA"
        ]
        ...
        for predictor in predictor_list:
            ...
            try:
                ...
                elif predictor in protein_biomarkers:
                    path = os.path.join(self.split_seed_path, f"X_{data_split}_Proteomics.feather")
                    proteomics_full = pd.read_feather(path)
                    x_df = proteomics_full[['eid', predictor]]
                elif predictor in metabolite_biomarkers:
                    path = os.path.join(self.split_seed_path, f"X_{data_split}_Metabolomics.feather")
    ...
    def run_analysis(self):
        ...
        all_biomarkers = metabolite_biomarkers
        predictor_sets = []
        for biomarker in all_biomarkers:
            predictor_sets.append([biomarker])
            predictor_sets.append(["PANEL", biomarker])
        
        total_predictor_sets = len(predictor_sets)
        all_results = []
        for outcome in outcomes_to_run:
            # Record start time for the current outcome
            ...
            for i, combo in enumerate(predictor_sets):
                ...
                if df_baseline_full is None or df_baseline_full.empty: continue
        ...
        if all_results:
            ...
            results_filename = os.path.join(self.save_dir, "cindex_individual_biomarker_metabolite_bootstrap_ci_summary.csv")
...
if __name__ == '__main__':
    ...
    log_filename = os.path.join(LOG_DIR, "CoxPH/Bootstrap_CI_Individual_Biomarker_Metabolite.log")
