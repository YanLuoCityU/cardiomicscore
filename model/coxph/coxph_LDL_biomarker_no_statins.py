...
class PointEstimateEvaluator:
    ...
    def run_analysis(self):
        ...
        for outcome in outcomes_to_run:
            ...
            for combo in predictor_sets:
                ...
                if df_baseline.empty:
                    self.logger.error(f"Baseline data '{baseline_name}' is empty after filtering. Skipping.")
                    continue
                ...
                df_combo = df_combo[df_combo['eid'].isin(eids_no_treatment)]
                if df_combo.empty:
                ...
                # --- NEW: Remove the treatment variable before fitting the model ---
                ...
                self.logger.info(f"Using {len(common_eids)} common samples (post-filtering) for comparison.")
                if len(common_eids) < 50: 
                    self.logger.warning(f"Sample size too small ({len(common_eids)}) after filtering. Skipping.")
                    continue
        ...
        if all_results:
            ...
            results_filename = os.path.join(self.cindex_save_dir, "cindex_LDL_biomarker_no_statins_summary.csv")
...
if __name__ == '__main__':
    ...
    log_filename = os.path.join(LOG_DIR, "CoxPH/Point_Estimates_LDL_Biomarker_No_Statins.log")
