...
class PointEstimateEvaluator:
    ...
    It calculates C-index on an external test set for different predictor combinations.
    """
    ...
    def _load_model_data(self, predictor_list: list, outcome: str, data_split: str) -> pd.DataFrame:
        ...
        """
        self.logger.info(f"Loading '{data_split}' data for combination: {predictor_list} (Outcome: {outcome})")
    ...
    def run_analysis(self):
        ...
        if all_results:
            ...
            results_filename = os.path.join(self.cindex_save_dir, f"cindex_{file_suffix}_summary.csv")
...
if __name__ == '__main__':
    ...
    for model_name in score_models_to_run:
        ...
        log_filename = os.path.join(LOG_DIR, f"CoxPH/Point_Estimates_{log_suffix}.log")
        
        os.makedirs(os.path.dirname(log_filename), exist_ok=True)
        
        logger_main = logging.getLogger(f"PointEstimateScript_{model_name}")
        logger_main.setLevel(logging.INFO)
        if logger_main.hasHandlers():
