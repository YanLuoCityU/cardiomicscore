...
class BaseTrainer:
    def __init__(self, config, model, optimizer, lr_scheduler,
        ...
        # self.checkpoint_path = f"{model_dir}/{self.config['predictor_set']}_tuning_best_model_trial_{self.config['trial_number']}.pth"
    ...
    def _save_checkpoint(self, epoch, best_cindex):
        ...
        # self.logger.info(f"Saving model checkpoint at epoch {epoch} to {self.checkpoint_path}") # Optional: can make logs too noisy during tuning
...
config = {
    ...
    'data_loader': {
        'batch_size': {'train': 1024, 'val': 1024}, 'shuffle_train': True,
        'num_workers': 16, 'seed_to_split': 250901,
    },
    'trainer': {'epochs': 50, 'early_stop': 50, 'lr': 1e-4, 'weight_decay': 0},
...
def objective(trial: optuna.trial.Trial) -> float:
    ...
    # activation = trial.suggest_categorical('activation', ['nn.ELU'])
