...
def death_min_dates(death_df, diag_list, rename):
    ...
    diag_list is the list of interested diagnoses.
    '''
    condition_columns = death_df.columns[2:]
...
output_dir = '/your path/cardiomicscore/data/processed/outcomes/'
...
vhd_death_df = death_min_dates(death_df, vhd_icd10_list, 'vhd_death_date')
...
vte_death_df = death_min_dates(death_df, vte_icd10_list, 'vte_death_date')
...
# Valve heart disease
vhd_df = pd.merge(outcomes_info_df, vhd_icd10_df, how = 'left', on =['eid'])
vhd_df = pd.merge(vhd_df, vhd_icd9_df, how = 'left', on =['eid'])
vhd_df = pd.merge(vhd_df, vhd_death_df, how = 'left', on =['eid'])
vhd_df['vhd_date'] = vhd_df[['vhd_icd10_date', 'vhd_icd9_date', 'vhd_death_date']].min(axis=1)
vhd_df = define_event_time(vhd_df, 'vhd')
...
# Venous thromboembolism
vte_df = pd.merge(outcomes_info_df, vte_icd10_df, how = 'left', on =['eid'])
vte_df = pd.merge(vte_df, vte_icd9_df, how = 'left', on =['eid'])
vte_df = pd.merge(vte_df, vte_opcs_df, how = 'left', on =['eid'])
vte_df = pd.merge(vte_df, vte_death_df, how = 'left', on =['eid'])
vte_df['vte_date'] = vte_df[['vte_icd10_date', 'vte_icd9_date', 'vte_opcs_date', 'vte_death_date']].min(axis=1)
vte_df = define_event_time(vte_df, 'vte')
...
cvd_df = pd.merge(cvd_df, vhd_df[['eid', 'vhd', 'vhd_date']], how = 'left', on =['eid'])
...
cvd_df = pd.merge(cvd_df, vte_df[['eid', 'vte', 'vte_date']], how = 'left', on =['eid'])
cvd_df['cvd_date'] = cvd_df[['cad_date', 'stroke_date', 'vhd_date', 'hf_date', 'af_date', 'va_date', 'pad_date', 'aaa_date', 'vte_date']].min(axis=1)
...
outcomes_df = pd.merge(outcomes_df, vhd_df[['eid', 'vhd', 'vhd_date', 'bl2vhd_yrs']], how = 'left', on =['eid'])
...
outcomes_df = pd.merge(outcomes_df, vte_df[['eid', 'vte', 'vte_date', 'bl2vte_yrs']], how = 'left', on =['eid'])
