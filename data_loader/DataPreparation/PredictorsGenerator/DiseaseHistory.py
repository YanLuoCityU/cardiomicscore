...
mi_20002_list = [1075]
mi_icd10_list = ['I21', 'I210', 'I211', 'I212', 'I213', 'I214', 'I219', 
                  'I22', 'I220', 'I221', 'I228', 'I229', 
                  'I23', 'I230', 'I231', 'I232', 'I233', 'I234', 'I235', 'I236', 'I238', 
                  'I241', 'I252']
mi_icd9_list = ['410', '4100', '4101', '4102', '4103', '4104', '4105', '4106', '4107', '4108', '4109', 
                '411', '4110', '4111', '4118', '412', '4129', '42979']
...
## valve heart disease
vhd_20002_list = [1488, 1489, 1490, 1584, 1585, 1586, 1587]
vhd_icd10_list = ['I05', 'I050', 'I051', 'I052', 'I058', 'I059', 
                  'I06', 'I060', 'I061', 'I062', 'I068', 'I069', 
                  'I07', 'I070', 'I071', 'I072', 'I078', 'I079', 
                  'I08', 'I080', 'I081', 'I082', 'I083', 'I088', 'I089', 
                  'I34', 'I340', 'I341', 'I342', 'I348', 'I349', 
                  'I35', 'I350', 'I351', 'I352', 'I358', 'I359', 
                  'I36', 'I360', 'I361', 'I362', 'I368', 'I369', 
                  'I37', 'I370', 'I371', 'I372', 'I378', 'I379', 
                  'I390', 'I391', 'I393', 'I394']
vhd_icd9_list = ['394', '395', '396', '4240', '4241', '4243']
...
vte_20002_list = [1068, 1093, 1094]
vte_icd10_list = ['I26', 'I260', 'I269', 'I80', 'I800', 'I801', 'I802', 'I803', 'I808', 'I809', 'I81', 'I820', 'I822', 'I823', 'I828', 'I829']
vte_icd9_list = ['4151', '451', '4510', '4511', '4512', '4518', '4519', '4532', '4538']
vte_opcs_list = ['L791', 'L902']
...
mi_20002_df = self_diag(diag_df, mi_20002_list, 20002, 'mi')
vhd_20002_df = self_diag(diag_df, vhd_20002_list, 20002, 'vhd')
...
vte_20002_df = self_diag(diag_df, vte_20002_list, 20002, 'vte')
del diag_df
gc.collect()
...
mi_reported_df = mi_20002_df.copy()
mi_reported_df.rename(columns = {'mi_20002': 'mi_self_report'}, inplace=True)
...
vte_reported_df = pd.merge(vte_20002_df, health_outcomes_df[['eid', '6152-0.0']], how = 'left', on =['eid'])
vte_reported_df['vte_self_report'] = ((vte_reported_df['vte_20002'] == 1) |
                                    (vte_reported_df['6152-0.0'] == 5) |
                                    (vte_reported_df['6152-0.0'] == 7)).astype(int)

del diab_20002_df, hypt_20002_df, cad_20002_df, mi_20002_df, hf_20002_df, af_20002_df, stroke_20002_df, pad_20002_df, vte_20002_df
...
mi_icd10_df = diag_min_dates(diag_df, date_df, mi_icd10_list, 'mi_icd10_date')
stroke_icd10_df = diag_min_dates(diag_df, date_df, stroke_icd10_list, 'stroke_icd10_date')
vhd_icd10_df = diag_min_dates(diag_df, date_df, vhd_icd10_list, 'vhd_icd10_date')
...
vte_icd10_df = diag_min_dates(diag_df, date_df, vte_icd10_list, 'vte_icd10_date')
del diag_df, date_df
gc.collect()
...
mi_icd9_df = diag_min_dates(diag_df, date_df, mi_icd9_list, 'mi_icd9_date')
stroke_icd9_df = diag_min_dates(diag_df, date_df, stroke_icd9_list, 'stroke_icd9_date')
vhd_icd9_df = diag_min_dates(diag_df, date_df, vhd_icd9_list, 'vhd_icd9_date')
...
vte_icd9_df = diag_min_dates(diag_df, date_df, vte_icd9_list, 'vte_icd9_date')
del diag_df, date_df
gc.collect()
...
vte_opcs_df = diag_min_dates(diag_df, date_df, vte_opcs_list, 'vte_opcs_date')
del diag_df, date_df
gc.collect()
...
# Myocardial infarction
mi_df = pd.merge(pc_df, mi_icd10_df, how = 'left', on =['eid'])
mi_df = pd.merge(mi_df, mi_icd9_df, how = 'left', on =['eid'])
mi_df = pd.merge(mi_df, mi_reported_df, how = 'left', on =['eid'])
mi_df['mi_date'] = mi_df[['mi_icd10_date', 'mi_icd9_date']].min(axis=1)
mi_df['bl2mi_yrs'] = get_date_interval('bl_date', 'mi_date', mi_df, 'year')
mi_df['mi_hist'] = ((mi_df['mi_self_report'] == 1) | (mi_df['bl2mi_yrs'] < 0)).astype(int)
del mi_icd10_df, mi_icd9_df, mi_reported_df
gc.collect()
...
# Valve heart disease
vhd_df = pd.merge(pc_df, vhd_icd10_df, how = 'left', on =['eid'])
vhd_df = pd.merge(vhd_df, vhd_icd9_df, how = 'left', on =['eid'])
vhd_df['vhd_date'] = vhd_df[['vhd_icd10_date', 'vhd_icd9_date']].min(axis=1)
vhd_df['bl2vhd_yrs'] = get_date_interval('bl_date', 'vhd_date', vhd_df, 'year')
vhd_df['vhd_hist'] = (vhd_df['bl2vhd_yrs'] < 0).astype(int)
del vhd_icd10_df, vhd_icd9_df
gc.collect()
...
# Venous thromboembolism
vte_df = pd.merge(pc_df, vte_icd10_df, how = 'left', on =['eid'])
vte_df = pd.merge(vte_df, vte_icd9_df, how = 'left', on =['eid'])
vte_df = pd.merge(vte_df, vte_opcs_df, how = 'left', on =['eid'])
vte_df = pd.merge(vte_df, vte_reported_df, how = 'left', on =['eid'])
vte_df['vte_date'] = vte_df[['vte_icd10_date', 'vte_icd9_date', 'vte_opcs_date']].min(axis=1)
vte_df['bl2vte_yrs'] = get_date_interval('bl_date', 'vte_date', vte_df, 'year')
vte_df['vte_hist'] = ((vte_df['vte_self_report'] == 1) | (vte_df['bl2vte_yrs'] < 0)).astype(int)
del vte_icd10_df, vte_icd9_df, vte_opcs_df, vte_reported_df
gc.collect()
...
feature_df = pd.merge(feature_df, mi_df[['eid', 'mi_hist']], how = 'left', on =['eid'])
feature_df = pd.merge(feature_df, stroke_df[['eid', 'stroke_hist']], how = 'left', on =['eid'])
feature_df = pd.merge(feature_df, vhd_df[['eid', 'vhd_hist']], how = 'left', on =['eid'])
...
feature_df = pd.merge(feature_df, vte_df[['eid', 'vte_hist']], how = 'left', on =['eid'])
