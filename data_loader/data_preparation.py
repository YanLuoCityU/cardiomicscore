...
class UKBiobankData():
    def __init__(self,
        ...
        log_filename = join(log_prep_dir, f'UKBiobankData_{self.seed}.log')
        ...
        self.predictor_set_mapping = {
            'ASCVD': self.dict_predictor['ASCVD'], 'SCORE2': self.dict_predictor['SCORE2'],
            'AgeSex': ['age', 'male'], 'Clinical': self.dict_predictor['clinical'],
            'PANEL': self.dict_predictor['PANEL'],
            'Genomics': self.dict_predictor['genomics'], 'Metabolomics': self.dict_predictor['metabolomics'],
            'Proteomics': self.dict_predictor['proteomics'],
            'Metabolomics_no_statins': self.dict_predictor['metabolomics'] 
        }
        self.continuous_predictor_mapping = {
            'ASCVD': ['age', 'sbp', 'total_cl', 'hdl_cl'], 'SCORE2': ['age', 'sbp', 'total_cl', 'hdl_cl'],
            'AgeSex': ['age'], 'Clinical': self.clinical_continuous_list,
        ...
        self.categorical_predictor_mapping = {
            ...
            'SCORE2': ['male', 'current_smoking', 'diab_hist'], 'AgeSex': ['male'],
            'Clinical': self.clinical_categorical_list, 'PANEL': self.clinical_categorical_list,
            'Genomics': [], 'Metabolomics': [], 'Proteomics': [], 'Metabolomics_no_statins': []
        }
        ...
        self.logger.info(f"Pre-existing disease columns: {history_cols}")
        ...
        # MODIFICATION 1: The line below was changed from self.excluded_df
        ...
        # Create the new 'Metabolomics_no_statins' pool
        self.logger.info("Creating TRAIN/VAL/INTERNAL TEST pool for predictor set: Metabolomics_no_statins.")
        pool_with_meds = pd.merge(self.excluded_metabolomics_pool, self.medication_df[['eid', 'lipidlower']], on='eid', how='left')
        
        missing_statin_info = pool_with_meds['lipidlower'].isnull().sum()
        if missing_statin_info > 0:
            self.logger.info(f"  ..for 'no statins' pool, found {missing_statin_info} participants with missing lipidlower information. Assuming they are not on statins (lipidlower=0).")
            pool_with_meds['lipidlower'].fillna(0, inplace=True)

        original_count = len(pool_with_meds)
        self.excluded_metabolomics_no_statins_pool = pool_with_meds[pool_with_meds['lipidlower'] != 1].drop(columns=['lipidlower'])
        excluded_count = original_count - len(self.excluded_metabolomics_no_statins_pool)
        self.logger.info(f"  ..excluded {excluded_count} participants on lipid-lowering medication. {len(self.excluded_metabolomics_no_statins_pool)} remaining.")
        self.logger.info(f"Finished creating pool for Metabolomics_no_statins. Final shape: {self.excluded_metabolomics_no_statins_pool.shape}.\n")
    ...
    def load_data(self):
        ...
        # Filter the main metabolomics dataframe using the generated list
    ...
    def get_predictors_dict(self):
        ...
        dict_predictor['blood_count'] = self.blood_count_continuous_list
        ...
        dict_predictor['clinical'] = (dict_predictor['demographic'] + dict_predictor['lifestyle'] + dict_predictor['familyhist'] + dict_predictor['phymeasure'] + dict_predictor['medication'] + dict_predictor['diseases'])
        dict_predictor['PANEL'] = dict_predictor['clinical'] + dict_predictor['blood_count']
    ...
    def merge_clinical_predictors(self):
        self.logger.info('Merging clinical, panel, and risk score predictors.')
        ...
        all_clinical_related_cols = list(dict.fromkeys(self.dict_predictor['PANEL'] + self.dict_predictor['ASCVD'] + self.dict_predictor['SCORE2'] + ['male', 'age']))
        ...
        self.logger.info(f"Final merged clinical/panel df shape after column selection: {self.merge_clinical_df.shape}")
    ...
    def exclusion_subcohort(self, predictors_df, main_test_set_eids, preexisting_disease_eids, predictor_set=None):
        ...
        # MODIFICATION 2: Added this new block for proteomics QC
    ...
    def _log_split_stats(self, df, set_name):
        """Helper function to log sample size and event counts for a dataset."""
        if df.empty:
            self.logger.info(f"{set_name}: 0 participants.")
            return
        
        # Merge with outcomes to get event flags
    ...
    def _create_splits(self, pool_df, pool_name):
        """Splits a data pool into internal test, train, and validation sets based on region."""
        self.logger.info("\n")
    ...
    def _split_and_prepare_data(self):
        """Orchestrates the new splitting strategy for all data pools."""
        self.X_train_clinical_df, self.X_val_clinical_df, self.X_internal_test_clinical_df = self._create_splits(self.excluded_clinical_pool, "Clinical/PANEL")
        ...
        self.X_train_metabolomics_no_statins_df, self.X_val_metabolomics_no_statins_df, self.X_internal_test_metabolomics_no_statins_df = self._create_splits(self.excluded_metabolomics_no_statins_pool, "Metabolomics_no_statins")
    ...
    def _fit_preprocessors(self, df_to_fit_on_full, predictor_set_name):
        self.logger.info(f"Fitting preprocessors for predictor set: {predictor_set_name}")
        relevant_predictor_cols = self.predictor_set_mapping.get(predictor_set_name, [])
        cols_for_fit_prep = ['eid'] + [col for col in relevant_predictor_cols if col in df_to_fit_on_full.columns]
        df_fit = df_to_fit_on_full[list(dict.fromkeys(cols_for_fit_prep))].copy().set_index('eid')
        continuous_cols = [col for col in self.continuous_predictor_mapping.get(predictor_set_name, []) if col in df_fit.columns]
        categorical_cols = [col for col in self.categorical_predictor_mapping.get(predictor_set_name, []) if col in df_fit.columns]
        
        # imputation_transformers = [('num_impute', SimpleImputer(strategy='mean'), continuous_cols)] if continuous_cols else []
        imputation_transformers = [('num_impute', KNNImputer(n_neighbors=5), continuous_cols)] if continuous_cols else []
        if categorical_cols: imputation_transformers.append(('cat_impute', SimpleImputer(strategy='most_frequent'), categorical_cols))
        imputer_ct = ColumnTransformer(transformers=imputation_transformers, remainder='passthrough').fit(df_fit)
        
        imputed_cols = continuous_cols + categorical_cols + [c for c in df_fit.columns if c not in continuous_cols and c not in categorical_cols]
        df_imputed_fit = pd.DataFrame(imputer_ct.transform(df_fit), columns=imputed_cols, index=df_fit.index)
        
        transformation_transformers = []
        if continuous_cols: transformation_transformers.append(('num_scale', StandardScaler(), continuous_cols))
        if categorical_cols: transformation_transformers.append(('cat_onehot', OneHotEncoder(drop='first', handle_unknown='ignore', sparse_output=False), categorical_cols))
        preprocessor_ct = ColumnTransformer(transformers=transformation_transformers, remainder='passthrough').fit(df_imputed_fit)
        
        if 'num_scale' in preprocessor_ct.named_transformers_:
            scaler = preprocessor_ct.named_transformers_['num_scale']
            scaler_params_df = pd.DataFrame({'feature': continuous_cols, 'mean': scaler.mean_, 'variance': scaler.var_})
            params_path = join(self.split_seed_dir, f"{predictor_set_name}_scaler_params.csv")
            scaler_params_df.to_csv(params_path, index=False)
            self.logger.info(f"Saved scaler parameters for {predictor_set_name} to {params_path}")
            
        return imputer_ct, preprocessor_ct

    def _apply_preprocessors(self, df_to_transform_full, predictor_set_name, fitted_imputer_ct, fitted_preprocessor_ct, save_path_prefix):
        if df_to_transform_full.empty:
            self.logger.warning(f"Skipping preprocessing for {save_path_prefix} because the input dataframe is empty.")
            return
        
        self.logger.info(f"Applying preprocessors for predictor set: {predictor_set_name} to create {save_path_prefix}")
        relevant_cols = self.predictor_set_mapping.get(predictor_set_name, [])
        cols_for_transform = ['eid'] + [col for col in relevant_cols if col in df_to_transform_full.columns]
        df_transform = df_to_transform_full[list(dict.fromkeys(cols_for_transform))].copy().set_index('eid')
        
        imputed_array = fitted_imputer_ct.transform(df_transform)
        imputed_cols = [c.split('__')[-1] for c in fitted_imputer_ct.get_feature_names_out()]
        df_imputed = pd.DataFrame(imputed_array, columns=imputed_cols, index=df_transform.index)
        
        transformed_array = fitted_preprocessor_ct.transform(df_imputed)
        feature_names = [c.split('__')[-1] for c in fitted_preprocessor_ct.get_feature_names_out()]
        df_transformed = pd.DataFrame(transformed_array, columns=feature_names, index=df_imputed.index)
        
        file_path = join(self.split_seed_dir, f"{save_path_prefix}.feather")
        df_transformed.reset_index().to_feather(file_path)
        self.logger.info(f"Saved processed dataset to {file_path}")

    def _process_special_set(self, df_to_transform_full, df_to_fit_on_full, predictor_set_name, save_path_prefix):
        if df_to_transform_full.empty:
            self.logger.warning(f"Skipping special processing for {save_path_prefix} because the input dataframe is empty.")
            return

        self.logger.info(f"Starting special processing for {predictor_set_name}, target: {save_path_prefix}")
        relevant_cols = self.predictor_set_mapping.get(predictor_set_name)
        
        cols_to_check = ['eid'] + relevant_cols
        missing_in_fit = [c for c in cols_to_check if c not in df_to_fit_on_full.columns]
        missing_in_transform = [c for c in cols_to_check if c not in df_to_transform_full.columns]
        if missing_in_fit or missing_in_transform:
            self.logger.error(f"Missing columns for {predictor_set_name}. In fit: {missing_in_fit}, In transform: {missing_in_transform}. Skipping.")
            return

        df_fit_subset = df_to_fit_on_full[cols_to_check].copy().set_index('eid')
        df_transform_subset = df_to_transform_full[cols_to_check].copy().set_index('eid')
        
        cont_cols = self.continuous_predictor_mapping.get(predictor_set_name, [])
        cat_cols = self.categorical_predictor_mapping.get(predictor_set_name, [])
        
        imputer_ct = ColumnTransformer(transformers=[('num', KNNImputer(n_neighbors=5), cont_cols), ('cat', SimpleImputer(strategy='most_frequent'), cat_cols)], remainder='passthrough').fit(df_fit_subset)
        imputed_array = imputer_ct.transform(df_transform_subset)
        imputed_cols = cont_cols + cat_cols
        df_imputed = pd.DataFrame(imputed_array, columns=imputed_cols, index=df_transform_subset.index)
        
        if predictor_set_name == 'ASCVD': df_processed = self._ascvd_variable_transform(df_imputed)
        elif predictor_set_name == 'SCORE2': df_processed = self._score2_variable_transform(df_imputed)
        else: df_processed = df_imputed
            
        file_path = join(self.split_seed_dir, f"{save_path_prefix}.feather")
        df_processed.reset_index().to_feather(file_path)
        self.logger.info(f"Saved processed {predictor_set_name} dataset to {file_path}")

    def _process_and_save_datasets(self):
        self.logger.info("\n")
        self.logger.info("="*80)
        self.logger.info("Starting Final Data Processing and Saving Stage")
        self.logger.info("="*80)
        
        fit_sources = {
            'AgeSex': self.excluded_clinical_pool,
            'Clinical': self.excluded_clinical_pool,
            'PANEL': self.excluded_clinical_pool,
            'Genomics': self.excluded_genomics_pool,
            'Proteomics': self.X_train_proteomics_df, 
            'Metabolomics': self.X_train_metabolomics_df,
            'Metabolomics_no_statins': self.X_train_metabolomics_no_statins_df
        }
        
        self.fitted_imputers, self.fitted_transformers = {}, {}

        self.logger.info("\n")
        self.logger.info("--- Fitting all preprocessors on respective data sources ---")
        for p_set, df_fit in fit_sources.items():
            if df_fit.empty:
                self.logger.warning(f"Skipping fitting for {p_set} because the fit source dataframe is empty.")
                continue
            imputer, transformer = self._fit_preprocessors(df_fit, p_set)
            self.fitted_imputers[p_set], self.fitted_transformers[p_set] = imputer, transformer

        sets_to_skip_splits = ['AgeSex', 'Clinical', 'PANEL', 'Genomics', 'ASCVD', 'SCORE2']

        for p_set in self.predictor_set_mapping.keys():
            self.logger.info("\n")
            self.logger.info(f"--- Processing all splits for: {p_set} ---")
            
            is_clinical_type = p_set in ['AgeSex', 'Clinical', 'PANEL', 'ASCVD', 'SCORE2']
            if is_clinical_type:
                df_train = self.X_train_clinical_df
                df_val = self.X_val_clinical_df
                df_internal_test = self.X_internal_test_clinical_df
            else:
                df_train = getattr(self, f"X_train_{p_set.lower()}_df")
                df_val = getattr(self, f"X_val_{p_set.lower()}_df")
                df_internal_test = getattr(self, f"X_internal_test_{p_set.lower()}_df")
            df_external_test = self.X_external_test_main_df

            if p_set in ['ASCVD', 'SCORE2']:
                df_fit_special = self.excluded_clinical_pool
                
                if p_set not in sets_to_skip_splits:
                    self._process_special_set(df_train, df_fit_special, p_set, f"X_train_{p_set}")
                    self._process_special_set(df_val, df_fit_special, p_set, f"X_val_{p_set}")
                    self._process_special_set(df_internal_test, df_fit_special, p_set, f"X_internal_test_{p_set}")
                self._process_special_set(df_external_test, df_fit_special, p_set, f"X_external_test_{p_set}")
            else:
                if p_set not in self.fitted_imputers:
                    self.logger.warning(f"No fitted preprocessor for {p_set}. Skipping.")
                    continue
                imputer, transformer = self.fitted_imputers[p_set], self.fitted_transformers[p_set]
                
                if p_set not in sets_to_skip_splits:
                    self._apply_preprocessors(df_train, p_set, imputer, transformer, f"X_train_{p_set}")
                    self._apply_preprocessors(df_val, p_set, imputer, transformer, f"X_val_{p_set}")
                    self._apply_preprocessors(df_internal_test, p_set, imputer, transformer, f"X_internal_test_{p_set}")
                self._apply_preprocessors(df_external_test, p_set, imputer, transformer, f"X_external_test_{p_set}")

            if p_set not in sets_to_skip_splits:
                y_train, e_train = self._get_y_e_sets(df_train['eid'])
                y_val, e_val = self._get_y_e_sets(df_val['eid'])
                y_internal_test, e_internal_test = self._get_y_e_sets(df_internal_test['eid'])
                
                y_train.to_feather(join(self.split_seed_dir, f'y_train_{p_set}.feather'))
                e_train.to_feather(join(self.split_seed_dir, f'e_train_{p_set}.feather'))
                y_val.to_feather(join(self.split_seed_dir, f'y_val_{p_set}.feather'))
                e_val.to_feather(join(self.split_seed_dir, f'e_val_{p_set}.feather'))
                y_internal_test.to_feather(join(self.split_seed_dir, f'y_internal_test_{p_set}.feather'))
                e_internal_test.to_feather(join(self.split_seed_dir, f'e_internal_test_{p_set}.feather'))
            else:
                self.logger.info(f"Skipping train/val/internal_test y/e file saving for {p_set}.")

        y_external_test, e_external_test = self._get_y_e_sets(self.X_external_test_main_df['eid'])
        y_external_test.to_feather(join(self.split_seed_dir, 'y_external_test.feather'))
        e_external_test.to_feather(join(self.split_seed_dir, 'e_external_test.feather'))
        self.logger.info("\n")
        self.logger.info("Saved common y/e files for the external test set.")
    ...
    def _ascvd_variable_transform(self, df):
        ...
        for column in ['age', 'sbp', 'antihypt', 'diab_hist', 'current_smoking', 'totalcl_hdlcl']:
            if column != 'black': df[f'black_{column}'] = df['black'] * df[column]
    ...
    def _score2_variable_transform(self, df):
        ...
        for column in df.columns:
            if column != 'age': df[f'age_{column}'] = df['age'] * df[column]
        return df
